...
function hasBad(s) { const t = String(s).replace(/\s/g, "").toLowerCase(); return BADWORDS.some(function (w) { return t.indexOf(w) >= 0; }); }
function clean(s, n) {
...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    // 봇이 채우는 숨은 입력칸(hp)에 값이 있으면 조용히 버린다. 사람에게는 보이지 않는 칸이다.
    if (clean(body.hp, 40)) return res.status(200).json({ ok: true, configured: true, item: null });
    const msg = clean(body.msg, 1000);
    if (!msg) return res.status(400).json({ ok: false, error: "empty" });
    if (hasBad(msg) || hasBad(body.name) || hasBad(body.region)) {
      return res.status(200).json({ ok: false, configured: true, error: "badword" });
    }
